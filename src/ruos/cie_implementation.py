...
_REQUIRED_SECTION_FIELDS = {"section_id","component","dom","css","interaction_hooks","motion_hooks","responsive","assets","qa_assertions","experience_pattern","scene_orchestration"}
...
def build_ui_implementation_contract(page: PageSpec, components: ComponentPlan, creative_director: Mapping[str, Any], experience_patterns: Mapping[str, Any] | None=None, scene_orchestration: Mapping[str, Any] | None=None) -> dict[str, Any]:
    ...
    experience_index={}; scene_index={}
    if isinstance(experience_patterns,Mapping):
        raw=experience_patterns.get("sections",[])
    ...
    if isinstance(scene_orchestration,Mapping):
        raw=scene_orchestration.get("sections",[])
        if isinstance(raw,list): scene_index={str(item.get("section_id")):item for item in raw if isinstance(item,Mapping) and item.get("section_id")}
    contracts=[]; blockers=[]
    for decision in decisions:
        ...
        component=components.for_section(section_id); motion=decision.get("motion") if isinstance(decision.get("motion"),Mapping) else None; interaction=str(decision.get("interaction","")).strip(); experience=experience_index.get(section_id); scene=scene_index.get(section_id)
