...
from .cie_visual_scene_runtime import render_visual_scene_css, render_visual_scene_runtime
...
def _industrial_layers(section: SectionSpec, spec: Mapping[str, Any]) -> str:
    experience = spec.get("experience_pattern", {}) if isinstance(spec.get("experience_pattern"), Mapping) else {}
...
def _section_markup(section: SectionSpec, component: Any, spec: Mapping[str, Any], index: int) -> str:
    dom = spec.get("dom", {}) if isinstance(spec.get("dom"), Mapping) else {}; interaction = spec.get("interaction_hooks", {}) if isinstance(spec.get("interaction_hooks"), Mapping) else {}; motion = spec.get("motion_hooks", {}) if isinstance(spec.get("motion_hooks"), Mapping) else {}; responsive = spec.get("responsive", {}) if isinstance(spec.get("responsive"), Mapping) else {}; css = spec.get("css", {}) if isinstance(spec.get("css"), Mapping) else {}; experience = spec.get("experience_pattern", {}) if isinstance(spec.get("experience_pattern"), Mapping) else {}; scenes = spec.get("scene_orchestration", {}) if isinstance(spec.get("scene_orchestration"), Mapping) else {}
    ...
    attributes = {"id": section.id, "class": f"ruos-section ruos-section--{section.kind} ruos-component ruos-component--{component.family} ruos-component--{variant} cie-native-section cie-pattern--{pattern}", "data-section-kind": section.kind, "data-component-id": component.id, "data-component-family": component.family, "data-component-variant": variant, "data-component-density": component.density, "data-component-emphasis": component.emphasis, "data-cie-contract": "native", "data-cie-section": section.id, "data-cie-variant": variant, "data-cie-experience": pattern, "data-cie-enhancement": experience.get("enhancement", "none"), "data-cie-layout": css.get("layout", "flow"), "data-cie-surface": css.get("surface", "default"), "data-cie-interaction": interaction.get("mode", "none"), "data-cie-motion": motion.get("effect", "none") if motion.get("enabled") else "none", "data-cie-touch-min": responsive.get("touch_targets_min_px", 44), "data-cie-scene": (scenes.get("scenes") or [{}])[0].get("id", "static"), "data-cie-state": scenes.get("initial_state", "static"), "data-cie-visual-stage": "true"}
...
/* CIE native contract-driven renderer + experience pattern engine */
.cie-native-section{container-type:inline-size}.cie-native-section :is(a,button,[role="button"]){min-inline-size:44px;min-block-size:44px}.cie-native-stage{isolation:isolate}
.cie-pattern--cinematic-scroll-stage .cie-native-stage{min-height:90svh;align-items:center}.cie-pattern--cinematic-scroll-stage [data-cie-depth-layer]{will-change:transform}
.cie-pattern--pinned-storytelling .ruos-section__content{position:relative}.cie-pattern--horizontal-journey [data-cie-collection]{grid-auto-flow:column;grid-auto-columns:minmax(18rem,34vw);overflow-x:auto;scroll-snap-type:x mandatory}.cie-pattern--horizontal-journey [data-cie-item]{scroll-snap-align:start}
...
@media (min-width:900px){.cie-pattern--pinned-storytelling .cie-native-stage{align-items:start}.cie-pattern--pinned-storytelling .ruos-section__content{position:sticky;top:calc(var(--header-height) + 2rem)}}
@media (max-width:760px){.cie-structure-anatomy{grid-template-columns:1fr}.cie-structure-anatomy__controls{grid-template-columns:1fr 1fr}.cie-native-stage{min-width:0}.cie-pattern--horizontal-journey [data-cie-collection]{grid-auto-columns:82vw}}
@media (prefers-reduced-motion:reduce){.cie-native-section,.cie-native-section *{animation-duration:.001ms!important;animation-iteration-count:1!important;transition-duration:.001ms!important;scroll-behavior:auto!important;transform:none!important}}
'''.strip(); return base.rstrip()+"\n\n"+native+"\n"+render_visual_scene_css(contract)

def render_runtime_from_contract(contract: Mapping[str, Any]) -> str:
    sections = _section_contracts(contract); payload = json.dumps({key:{"interaction":value.get("interaction_hooks",{}),"motion":value.get("motion_hooks",{}),"responsive":value.get("responsive",{}),"experience":value.get("experience_pattern",{}),"scenes":value.get("scene_orchestration",{}),"qa":value.get("qa_assertions",[])} for key,value in sections.items()},ensure_ascii=False,separators=(",",":"))
    native = f'''const RUOS_CIE_NATIVE={payload};
const cieReduce=matchMedia('(prefers-reduced-motion: reduce)').matches;
for(const [sectionId,contract] of Object.entries(RUOS_CIE_NATIVE)){{const section=document.getElementById(sectionId);if(!section)continue;section.dataset.cieRuntime='native';const min=Math.max(44,Number(contract.responsive.touch_targets_min_px||44));section.style.setProperty('--cie-touch-min',`${{min}}px`);for(const control of section.querySelectorAll('a,button,[role="button"]')){{control.style.minWidth='var(--cie-touch-min)';control.style.minHeight='var(--cie-touch-min)';}}const scenes=Array.isArray(contract.scenes.scenes)?contract.scenes.scenes:[];const applyScene=()=>{{if(!scenes.length)return;const r=section.getBoundingClientRect();const total=Math.max(1,r.height+innerHeight);const progress=Math.max(0,Math.min(1,(innerHeight-r.top)/total));const scene=scenes.find(item=>progress>=Number(item.range?.[0]||0)&&progress<=Number(item.range?.[1]||1))||scenes[scenes.length-1];section.dataset.cieScene=scene.id||'';section.dataset.cieState=scene.state||'';}};applyScene();if(!cieReduce)addEventListener('scroll',applyScene,{{passive:true}});}}
for(const anatomy of document.querySelectorAll('[data-cie-industrial-anatomy]')){{const output=anatomy.querySelector('[data-cie-hotspot-output]');for(const button of anatomy.querySelectorAll('[data-cie-hotspot]')){{button.addEventListener('click',()=>{{for(const peer of anatomy.querySelectorAll('[data-cie-hotspot]'))peer.setAttribute('aria-pressed','false');button.setAttribute('aria-pressed','true');const key=button.dataset.cieHotspot;const section=anatomy.closest('[data-cie-section]');if(section)section.dataset.cieState=key||section.dataset.cieState;for(const layer of anatomy.querySelectorAll('[data-layer]')){{layer.style.opacity=layer.dataset.layer===key?'1':'.28';layer.style.transform=layer.dataset.layer===key?'scale(1.04)':'scale(.98)';}}if(output)output.textContent=button.textContent||'';}});}}}}'''
    return render_runtime().rstrip()+"\n\n"+native+"\n"+render_visual_scene_runtime(contract)
