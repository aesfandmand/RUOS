...
def _section_markup(section: SectionSpec, component: Any, spec: Mapping[str, Any], index: int) -> str:
    dom = spec.get("dom", {}) if isinstance(spec.get("dom"), Mapping) else {}; interaction = spec.get("interaction_hooks", {}) if isinstance(spec.get("interaction_hooks"), Mapping) else {}; motion = spec.get("motion_hooks", {}) if isinstance(spec.get("motion_hooks"), Mapping) else {}; responsive = spec.get("responsive", {}) if isinstance(spec.get("responsive"), Mapping) else {}; css = spec.get("css", {}) if isinstance(spec.get("css"), Mapping) else {}; experience = spec.get("experience_pattern", {}) if isinstance(spec.get("experience_pattern"), Mapping) else {}
