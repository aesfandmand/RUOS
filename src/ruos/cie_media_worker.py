...
def _record(path: Path, project_root: Path) -> dict[str, Any]:
    try:
        ...
        uri = path.as_posix()
    return {"uri": uri, "bytes": path.stat().st_size, "kb": max(1, (path.stat().st_size + 1023) // 1024), "sha256": _sha256(path)}


def _source_path(project_root: Path, uri: object) -> Path | None:
    if not uri:
        return None
    candidate = Path(str(uri)); path = candidate if candidate.is_absolute() else project_root / candidate
    return path if path.is_file() else None


def _produce_image(asset: Mapping[str, Any], source: Path, output_dir: Path, project_root: Path) -> list[dict[str, Any]]:
    variants = asset.get("variants", []) if isinstance(asset.get("variants"), list) else []; produced: list[dict[str, Any]] = []
    with Image.open(source) as image:
        base = image.convert("RGB") if image.mode not in {"RGB", "RGBA"} else image.copy()
        for variant in variants:
            if not isinstance(variant, Mapping): continue
            fmt = str(variant.get("format", "")).lower(); width = int(variant.get("width", base.width) or base.width)
            if fmt not in {"webp", "avif"}: continue
            scale = min(1.0, width / max(1, base.width)); target = base if scale == 1.0 else base.resize((max(1, round(base.width * scale)), max(1, round(base.height * scale))), Image.Resampling.LANCZOS)
            path = output_dir / f"{asset['asset_id']}-{width}.{fmt}"
            try: target.save(path, format=fmt.upper(), quality=82, optimize=True)
            except Exception as exc: raise MediaProductionError(f"{asset['asset_id']}: failed to encode {fmt}: {exc}") from exc
...
def _produce_svg(asset: Mapping[str, Any], source: Path, output_dir: Path, project_root: Path) -> list[dict[str, Any]]:
    path = output_dir / f"{asset['asset_id']}.svg"; shutil.copy2(source, path)
    return [{"format": "svg", "mode": "original-vector", **_record(path, project_root), "status": "produced"}]


def _run(command: list[str]) -> None:
    completed = subprocess.run(command, capture_output=True, text=True)
    if completed.returncode != 0: raise MediaProductionError((completed.stderr or completed.stdout or "media tool failed").strip())


def _produce_video(asset: Mapping[str, Any], source: Path, output_dir: Path, project_root: Path) -> list[dict[str, Any]]:
    ffmpeg = shutil.which("ffmpeg")
    if not ffmpeg: return [{"status": "blocked", "reason": "ffmpeg-not-installed", "format": str(v.get("format", ""))} for v in asset.get("variants", []) if isinstance(v, Mapping)]
    produced: list[dict[str, Any]] = []
    for variant in asset.get("variants", []) if isinstance(asset.get("variants"), list) else []:
        if not isinstance(variant, Mapping): continue
        fmt = str(variant.get("format", "mp4")); max_width = int(variant.get("max_width", 1280) or 1280); path = output_dir / f"{asset['asset_id']}-{variant.get('profile','default')}.{fmt}"
...
def _produce_model(asset: Mapping[str, Any], source: Path, output_dir: Path, project_root: Path) -> list[dict[str, Any]]:
    tool = shutil.which("gltf-transform"); produced: list[dict[str, Any]] = []; variants = asset.get("variants", []) if isinstance(asset.get("variants"), list) else []
    for variant in variants:
        if not isinstance(variant, Mapping): continue
        lod = str(variant.get("lod", "high"))
        if lod == "high":
            path = output_dir / f"{asset['asset_id']}-high.glb"; shutil.copy2(source, path); produced.append({**dict(variant), **_record(path, project_root), "status": "produced"}); continue
        if lod == "poster": produced.append({**dict(variant), "status": "external-poster-required"}); continue
        if not tool: produced.append({**dict(variant), "status": "blocked", "reason": "gltf-transform-not-installed"}); continue
        path = output_dir / f"{asset['asset_id']}-{lod}.glb"; _run([tool, "optimize", str(source), str(path), "--compress", "meshopt"]); produced.append({**dict(variant), **_record(path, project_root), "status": "produced"})
    return produced
...
def validate_produced_media_budget(report: Mapping[str, Any], manifest: Mapping[str, Any]) -> dict[str, Any]:
    budgets = manifest.get("budgets", {}) if isinstance(manifest.get("budgets"), Mapping) else {}; mobile = budgets.get("mobile", {}) if isinstance(budgets.get("mobile"), Mapping) else {}; desktop = budgets.get("desktop", {}) if isinstance(budgets.get("desktop"), Mapping) else {}
    initial_kb = 0; total_kb = 0; failures: list[str] = []
    for asset in report.get("assets", []) if isinstance(report.get("assets"), list) else []:
        if not isinstance(asset, Mapping): continue
        produced = [v for v in asset.get("variants", []) if isinstance(v, Mapping) and v.get("status") == "produced"]
        asset_kb = sum(int(v.get("kb", 0) or 0) for v in produced); total_kb += asset_kb
        if asset.get("priority") == "high": initial_kb += asset_kb
        if asset.get("status") == "blocked": failures.append(f"{asset.get('asset_id','unknown')}: derivative production blocked")
    if initial_kb > int(mobile.get("initial_media_kb", 0) or 0): failures.append(f"produced critical media {initial_kb}KB exceeds mobile initial budget")
    if total_kb > int(desktop.get("deferred_media_kb", 0) or 0): failures.append(f"produced total media {total_kb}KB exceeds desktop deferred budget")
    return {"status": "pass" if not failures else "blocked", "failures": failures, "observed": {"produced_initial_kb": initial_kb, "produced_total_kb": total_kb}}
