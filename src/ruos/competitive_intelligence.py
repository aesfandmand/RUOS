...
class CompetitiveSignal:
    ...
    def payload(self) -> dict[str, str]:
        return {"source_id": self.source_id, "market": self.market, "signal_type": self.signal_type, "observation": self.observation, "implication": self.implication}
...
class CompetitorPageEvidence:
    ...
    def payload(self) -> dict[str, object]:
        return {"rank": self.rank, "requested_url": self.requested_url, "final_url": self.final_url, "domain": self.domain, "fetched_at": self.fetched_at, "content_sha256": self.content_sha256, "title": self.title, "excerpt": self.excerpt, "byte_length": self.byte_length}
...
class CompetitiveIntelligence:
    ...
    def payload(self) -> dict[str, object]:
        payload: dict[str, object] = {"page_slug": self.page_slug, "local_sources": list(self.local_sources), "global_sources": list(self.global_sources), "signals": [signal.payload() for signal in self.signals], "opportunity_gaps": list(self.opportunity_gaps), "non_copying_policy": self.non_copying_policy}
...
def _parse_discovery(research: ResearchBrief) -> tuple[str | None, str | None, tuple[DiscoveredCompetitor, ...]]:
    raw = (research.provenance or {}).get("search_discovery")
    ...
    for index, item in enumerate(rows, start=1):
        ...
        if rank != index:
            raise CompetitiveIntelligenceError("Discovery ranks must be contiguous")
...
def _parse_page_evidence(research: ResearchBrief) -> tuple[CompetitorPageEvidence, ...]:
    raw = (research.provenance or {}).get("competitor_page_evidence")
    if raw is None:
        return ()
    ...
    for index, item in enumerate(raw, start=1):
        ...
        evidence.append(CompetitorPageEvidence(int(item.get("rank", index)), requested_url, final_url, domain, str(item.get("fetched_at", "")).strip(), content_sha256, str(item.get("title", "")).strip(), str(item.get("excerpt", "")).strip(), int(item.get("byte_length", 0))))
...
def build_competitive_intelligence(page: PageSpec, research: ResearchBrief) -> CompetitiveIntelligence:
    ...
    return CompetitiveIntelligence(page.slug, local, global_refs, tuple(signals), gaps, "Use sources to derive principles and gaps; never copy layout, copywriting, code or branded assets.", provider, discovery_sha256, discovered, page_evidence)
